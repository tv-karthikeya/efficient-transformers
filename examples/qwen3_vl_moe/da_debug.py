...
if dist:
    ...
    if skip_vision:
        # for only LLM with DA
        decode_qpc_path = qpc_paths[0]
        prefill_qpc_path = qpc_paths[1]
        ...
        vision_qpc_path = qpc_paths[0]
        decode_qpc_path = qpc_paths[1]
        prefill_qpc_path = qpc_paths[2]
...
# generation_len =  torch.abs(CTX_LEN - input_len.max())
...
for i in range(generation_len - 2):
    ...
    for j in range(config.text_config.num_hidden_layers):
        # dif_k = decode_out[f"past_key.{j}_RetainedState"] -  loop_decode_inputs[f"past_key.{j}"]
        # dif_v = decode_out[f"past_value.{j}_RetainedState"] - loop_decode_inputs[f"past_value.{j}"]
...
#     outputs = lang_decode_session.run(lang_inputs)
...
# print(f"Average Prefill time a.k.a TTFT is= {round(prefill_time, 2)} sec")
# print(
#     f"\nDecode is= {round(decode_perf, 2)}  token/sec\
#         \nTotal is= {round(total_perf, 2)} token/sec\
#         \nTotal (E2E) inference time is= {round(total_time, 2)} sec"
# )
# print(generated_ids)
...
#  # Get first token
# lang_inputs["input_ids"] = outputs["logits"].argmax(2)
# print(f'>>>>>>>>>>>>> lang_inputs["input_ids"] : {lang_inputs["input_ids"]}')
