...
class QEffWanImageToVideoPipeline:
    ...
    def __init__(self, model, **kwargs):
        ...
        self.vae_encoder = model.vae.encode #TODO move to QAIC
    ...
    def prepare_latents(
        ...
        first_frame_mask = torch.repeat_interleave(first_frame_mask, dim=2, repeats=self.model.vae.config.scale_factor_temporal)
        mask_lat_size = torch.concat([first_frame_mask, mask_lat_size[:, :, 1:, :]], dim=2)
        mask_lat_size = mask_lat_size.view(batch_size, -1, self.model.vae.config.scale_factor_temporal, latent_height, latent_width)
